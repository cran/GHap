---
title: "The **GHap** Package (Version 1.2.0)"
...
Briefly, let $\mathbf{H}$ be the centered $n$ x $m$ matrix of HapGenotypes, where $n$ is the number of observations and $m$ is the number of HapAlleles. Also, we define $q$ as the inverse sum of variances in the columns of $\mathbf{H}$. The HapAllele covariance among individuals can be computed as:

$$\mathbf{K} = q\mathbf{HDH}'$$

where $\mathbf{D} = \text{diag}(d_i)$, and $d_i$ is the weight of HapAllele $i$ (default $d_i = 1$). Notice that this is a generalization of the SNP-based genomic relationship matrix (VanRaden, 2008). The example below computes a kinship matrix from HapGenotypes and plots the first two eigenvectors of a singular value decomposition (principal components analysis) of this matrix.
...
In this setting, the parameters we wish to estimate are $\mathbf{b}$, $\mathbf{u}$, $\sigma_u^2$ and $\sigma_e^2$, for which we assign the following prior distributions:
...
$$Pr(\mathbf{b}, \mathbf{u}, \mathbf{p}, \sigma_{u}^2, \sigma_{p}^2, \sigma_{e}^2 \mid \mathbf{y}) \propto Pr(\mathbf{y} \mid \mathbf{b}, \mathbf{u}, \mathbf{p}, \sigma_{u}^2, \sigma_{p}^2, \sigma_{e}^2)Pr(\mathbf{u} \mid \sigma_{u}^2)Pr(\mathbf{p} \mid \sigma_{p}^2)Pr(\sigma_{u}^2)Pr(\sigma_{e}^2)$$
...
The `ghap.blmm()` function accommodates repeated measurements through $\mathbf{Z}$ and heteroskedastic residuals through $\mathbf{W}$. The function can also take ordered categorical data as response. This is achieved by assuming that categories emerge from thresholds of a latent normal variable (i.e., "liability"). We use data augmentation to sample thresholds and observations from the underlying latent variable, which we then treat as responses in the main algorithm.
...
In the first case, we can use genomic control to recover p-values to an unbiased scale (Devlin and Roeder, 1999; Amin et al., 2007). However, not much can be done regarding the estimates of the effects. As a general recommendation, if the user is only interested in p-values, the ghap.assoc analysis should be sufficient. When effect estimates are of interest, the user can select genome-wide significant HapAlleles and include them as fixed effects in the full model in ghap.blmm. For the second case, a leave-one-chromosome-out (LOCO analysis) procedure can mitigate proximal contamination (Yang et al., 2014).

###GWAS treating HapBlocks as fixed effects

**GHap** also allows testing HapBlocks for association. If the argument type in `ghap.assoc()` is supplied with the option "HapBlock" instead of the default "HapAllele", all $m$ HapAlleles within a block are fitted simultaneously:

$$\hat{\mathbf{a}} = (\mathbf{H}'\mathbf{H})^{-1}\mathbf{H}'\mathbf{y}_\text{adj}$$

where $\mathbf{H}$ is the $n$ x $m$ sub-HapGenotypes matrix for the HapAlleles within the block. Then, given the explained ($ESS$) and the residual ($RSS$) sum of squares, an F test is computed instead of the chi-squared test:

$$F = \frac{ESS/\nu_1}{RSS/\nu_2}$$
$$ESS = (\mathbf{H}\hat{\mathbf{a}} - \hat{\mu}_y)'(\mathbf{H}\hat{\mathbf{a}} - \hat{\mu}_y)$$
$$RSS = (\mathbf{y}_\text{adj} - \mathbf{H}\hat{\mathbf{a}})'(\mathbf{y}_\text{adj} - \mathbf{H}\hat{\mathbf{a}})$$
$$\hat{\mu}_y = \frac{1}{N} \sum_{i=1}^{n} y_i$$
$$\nu_1 = m$$
$$\nu_2 = n - m$$

Under the null hypothesis that the variance due to the HapBlock is zero $F \sim F(\nu_1,\nu_2)$.
...
Recall the model fitted by the `ghap.blmm()` function:
...
###Appendix - Bayesian linear mixed model
...
**Model with permanent environmental effects, correlated random effects and heteroskedastic residuals**

The conditional posterior distribution of permanent environmental effects is identical to the uncorrelated effects discussed in this appendix. We can also incorporate covariance of random effects and heteroskedastic residuals into the model without effectively changing the full and conditional posterior distributions. The targeted model assumes:
...
Once the model is fitted, we can solve $\mathbf{\hat{u}} = \mathbf{L\mathbf{\hat{u}}^*}$ and $\mathbf{\hat{e}} = \mathbf{W}^{1/2}\mathbf{\hat{e}}^*$.

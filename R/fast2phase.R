...
ghap.fast2phase <- function(
  ...
  for(i in 1:length(switchout.files)){
    ...
    # Get sample info
    fp_split <- strsplit(fp$V1[idx], split = " ")
    samples <- matrix(unlist(fp_split), ncol = length(fp_split[[1]]), byrow = T)
    samples <- samples[,c(6,1)]
    
    # Check if samples are the same across files
    if(is.null(samples.old) == FALSE){
      if(identical(samples.old,samples) == FALSE){
  ...
  # Output samples file
  fwrite(x = as.data.table(samples), file = tmp.samples.file, col.names = FALSE,
         row.names = FALSE, sep = " ")
